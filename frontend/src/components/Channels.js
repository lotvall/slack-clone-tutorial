...
const dmChannel = ({ id, name }, teamId) => (
  ...
  <Link to={`/view-team/${teamId}/${id}`} key={`user-${id}`}>
    <SideBarListItem >
      <Bubble /> {name}
    </SideBarListItem>
  </Link>
...
export default ({
  teamName, teamId, username, userId, isOwner, channels, dmChannels, onAddChannelClick, onInvitePeopleClick, onDirectMessageClick
}) => (
  <ChannelWrapper>
    <PushLeft>
